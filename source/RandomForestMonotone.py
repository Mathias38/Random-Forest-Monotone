...
class RandomForestClassifier(object):
    ...
    def fit_with_monotony_drop_out(self, x, y, nb_rows, positive_vars, negative_vars, target_monotony_percent, nb_tests_mono_pos, nb_tests_mono_neg):
        ...
        while c < self.nb_trees:
            ...
            tree = DecisionTreeClassifier(max_depth = self.max_depth, max_features = 0.5)
            tree.fit(x, y)
            ct = ct + 1 
    ...
    def predict(self, feature):
        ...
        for tree in self.trees:
            predictions.append(tree.predict(np.array(feature).reshape(1,-1))[0])
    ...
    """       
    def print_all_variables_monotony(self, column_names, tree, positive_vars, negative_vars, target_monotony_percent, nb_tests_mono_pos, nb_tests_mono_neg) :
                
        for k in range(0, len(positive_vars)): 
            print(column_names[positive_vars[k]])
            print(self.get_var_mono_percent("pos", positive_vars[k], self.values[positive_vars[k]], tree, nb_tests_mono_pos[k], self.dtypes[positive_vars[k]]))
            
        for h in range(0, len(negative_vars)):
            print(column_names[negative_vars[h]])
            print(self.get_var_mono_percent("neg", negative_vars[h], self.values[negative_vars[h]], tree, nb_tests_mono_neg[h], self.dtypes[negative_vars[h]]))
